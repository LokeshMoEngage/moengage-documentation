...
<Info>
  The webhook accepts JSON. XML is not supported, and a request that sends it can fail with an Unsupported Media Type error. Set **Body Type** to **JSON** and send the `Content-Type` header as `application/json`.
</Info>
