...
<Update label="16th September 2026" description="SDK 11.2.0">
...
* **Static linking:** The InApp framework is now statically linked into the host binary instead of embedded as a dynamic framework. CocoaPods and Swift Package Manager integrations require no change.
* Manual xcframework integrations require two changes:
  * Set the framework to **Do Not Embed** and copy `MoEngageInApps.bundle` from the release zip into the app bundle. **Do Not Embed** copies nothing out of the framework, so without this step the app ships without its privacy manifest.
  * Add `-ObjC` to **Other Linker Flags**. The linker includes a statically linked module's code only where the app references a symbol it defines.

<span style={{ backgroundColor: 'rgba(212, 107, 8, 0.1)', color: '#D46B08', border: '1px solid #FFE58F', padding: '2px 10px', borderRadius: '12px', fontSize: '0.85em', fontWeight: 'bold' }}>Fixes</span>

* Fixed the module not being initialized in Swift Package Manager integrations, an issue introduced in SDK 11.1.0. Static linking dropped code that no app symbol referenced directly, including classes the SDK resolves by name and `@objcMembers` methods declared in extensions. The module is now built as a single object file, so referencing any part of it links all of it.
* Fixed race conditions in in-app stats tracking when the SDK is initialized multiple times in a process's lifecycle.

#### Cards
<span style={{ backgroundColor: 'rgba(0, 135, 103, 0.1)', color: '#008767', border: '1px solid #B5F5EC', padding: '2px 10px', borderRadius: '12px', fontSize: '0.85em', fontWeight: 'bold' }}>Improvements</span>

* **Static linking:** The Cards framework is now statically linked into the host binary instead of embedded as a dynamic framework. CocoaPods and Swift Package Manager integrations require no change.
* Manual xcframework integrations require two changes:
  * Set the framework to **Do Not Embed** and copy `MoEngageCards.bundle` from the release zip into the app bundle. **Do Not Embed** copies nothing out of the framework, so without this step the app ships without its privacy manifest.
  * Add `-ObjC` to **Other Linker Flags**. The linker includes a statically linked module's code only where the app references a symbol it defines.
...
* **Static linking:** The Inbox framework is now statically linked into the host binary instead of embedded as a dynamic framework. CocoaPods and Swift Package Manager integrations require no change.
* Manual xcframework integrations require two changes:
  * Set the framework to **Do Not Embed** and copy `MoEngageInbox.bundle` from the release zip into the app bundle. **Do Not Embed** copies nothing out of the framework, so without this step the app ships without its privacy manifest.
  * Add `-ObjC` to **Other Linker Flags**. The linker includes a statically linked module's code only where the app references a symbol it defines.
...
* **Static linking:** The Geofence framework is now statically linked into the host binary instead of embedded as a dynamic framework. CocoaPods and Swift Package Manager integrations require no change.
* Manual xcframework integrations require two changes:
  * Set the framework to **Do Not Embed** and copy `MoEngageGeofence.bundle` from the release zip into the app bundle. **Do Not Embed** copies nothing out of the framework, so without this step the app ships without its privacy manifest.
  * Add `-ObjC` to **Other Linker Flags**. The linker includes a statically linked module's code only where the app references a symbol it defines.
...
* **Static linking:** The RealTimeTrigger framework is now statically linked into the host binary instead of embedded as a dynamic framework. CocoaPods and Swift Package Manager integrations require no change.
* Manual xcframework integrations require two changes:
  * Set the framework to **Do Not Embed** and copy `MoEngageRealTimeTrigger.bundle` from the release zip into the app bundle. **Do Not Embed** copies nothing out of the framework, so without this step the app ships without its privacy manifest.
  * Add `-ObjC` to **Other Linker Flags**. The linker includes a statically linked module's code only where the app references a symbol it defines.
...
* **Static linking:** The Personalize framework is now statically linked into the host binary instead of embedded as a dynamic framework. CocoaPods and Swift Package Manager integrations require no change.
* Manual xcframework integrations require two changes:
  * Set the framework to **Do Not Embed** and copy `MoEngagePersonalization.bundle` from the release zip into the app bundle. **Do Not Embed** copies nothing out of the framework, so without this step the app ships without its privacy manifest.
  * Add `-ObjC` to **Other Linker Flags**. The linker includes a statically linked module's code only where the app references a symbol it defines.
...
* Fixed the app icon badge being one higher than the notification tray after a notification update. The extension now excludes the notification being replaced when computing the badge.
...
<Warning>
  MoEngage does not recommend SDK 11.1.0. In this version, the newly introduced static frameworks (InApps, Personalization, Cards, Inbox, Geofence, and RealTimeTrigger) do not link correctly, so they skip initialization and campaigns do not display to users. This affects Swift Package Manager and manual integrations. Upgrade to [SDK 11.2.0](/release-notes/sdks/ios#16th-september-2026), which includes all changes in 11.1.0 together with additional fixes.
</Warning>
