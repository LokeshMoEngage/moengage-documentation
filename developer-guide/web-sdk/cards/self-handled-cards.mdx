...
Each UI element in a card is a widget with an identifier that you pass to the click-tracking API. The table below maps the widget identifiers to the elements they represent.

#### Basic Card/Illustration Card

| Widget Id | Widget Type            | Widget Information                |
| --------- | ---------------------- | --------------------------------- |
| 0         | Image (type: image)    | Image widget in the card.         |
| 1         | Text (type: text)      | Header text for the card.         |
| 2         | Text (type: text)      | Message text for the card.        |
| 3         | Button (type: button)  | Call to action(CTA) for the card. |
