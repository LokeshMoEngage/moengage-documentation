---
title: "How to fix Token Drop? (Legacy)"
description: "Learn why push token drops occur in your Android app on SDK versions below 15.03.00 and how to fix them for reliable delivery."
---

<Warning>
  This page applies to MoEngage Android SDK versions below 15.03.00. For 15.03.00 and above, where the SDK registers devices for push through the Firebase Installation ID, refer to [How to Fix Installation ID Drop?](/developer-guide/android-sdk/troubleshooting-and-faqs/how-to-fix-installation-id-drop).
</Warning>
...
* Revisit [Push Token Registration and Display (Legacy)](/developer-guide/android-sdk/push/basic/push-token-registration-and-display-legacy) to check if the implementation for passing the token is properly done.
...
<Info>
  ...
  * MoEngage SDK already has a retry mechanism built in to reduce token drop. We recommend letting the MoEngage SDK handle token registration to keep token drop minimal (typically under 2%). Refer to [Push Token Registration and Display (Legacy)](/developer-guide/android-sdk/push/basic/push-token-registration-and-display-legacy) to learn more about letting the MoEngage SDK manage your push token.
</Info>
