...
- If these `DEBUG` validations crash your app while you upgrade, call `MoEngageSDKCore.sharedInstance.disableIntegrationValidator()` before initializing the SDK to opt out temporarily. Remove the call once the issues are fixed.
</Warning>
...
<Tabs>
  <Tab title="Provisional + Standard Push Notifications">
    ...
    class AppDelegate: UIResponder, UIApplicationDelegate {
        ...
        // To display standard notifications later in the user's journey, call this method after the app starts.
        func requestStandardPushPermission() {
    ...
    // To display standard notifications later in the user's journey, call this method after the app starts.
    - (void)requestStandardPushPermission {
...
By default, the SDK sets the app icon badge to the number of MoEngage notifications in Notification Center.

To manage the badge yourself, add the `MoEngageBadgeUpdateEnabled` key to your app's `Info.plist` and set it to `false`. This key is supported from iOS SDK [10.13.0](/release-notes/sdks/ios#25th-may-2026).
...
<Warning>
  `MoEngageSDKMessaging.disableBadgeReset(_:)` is deprecated from iOS SDK [10.13.0](/release-notes/sdks/ios#25th-may-2026) and has no effect. Remove any calls to it and use the `Info.plist` key instead. For more information, refer to [Custom Notification Handling](/developer-guide/ios-sdk/push/advanced/custom-notification-handling#disable-badge-reset).
</Warning>
