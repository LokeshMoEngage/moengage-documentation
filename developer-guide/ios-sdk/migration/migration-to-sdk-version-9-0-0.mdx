...
| `MoEngage.sharedInstance().setDisableBadgeReset(true)`                                                                   | `MoEngageSDKMessaging.sharedInstance.disableBadgeReset(true)`. Deprecated from SDK 10.13.0 and now a no-op. Use the `MoEngageBadgeUpdateEnabled` key in `Info.plist` instead. Refer to [Disable Badge Reset](/developer-guide/ios-sdk/push/advanced/custom-notification-handling#disable-badge-reset). |
