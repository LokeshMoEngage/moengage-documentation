...
<Note>
  ...
  - `MoEngageProperties(withAttributes:)` and `addAttribute(_:withName:)` accept `any Sendable` values instead of `Any`. Declare the attribute dictionary as `[String: any Sendable]`; `[String: Any]` does not compile.
