...
The first send of a Daily, Weekly, or Monthly flow is calculated as the earliest occurrence of your chosen recurrence pattern that falls on or after your configured Start Date and Send Time, not necessarily the Start Date itself. If the recurrence day you selected has already passed within that cycle relative to your Start Date, MoEngage skips that occurrence and moves the first send to the next matching cycle.

For example, if you set a Monthly flow with a Start Date of July 22 and choose to repeat on the 21st of every month, July 21 has already passed relative to July 22, so the first send happens on August 21, not July 21.

This is expected behavior, not an error. See [How do I confirm exactly when my flow's first campaign will send?](#how-do-i-confirm-exactly-when-my-flows-first-campaign-will-send) to verify the computed date before publishing.

## How do I confirm exactly when my flow's first campaign will send?

Once you've configured a Daily, Weekly, or Monthly schedule under **At fixed time**, MoEngage displays a confirmation line at the bottom of the scheduling panel, for example, *"Campaign will be next sent on Sat Aug 22 2026 11:30 PM."* This line reflects the actual computed first (or next) send date and time based on your current configuration, accounting for your Start Date, Send Time, and recurrence settings.

Always check this line before publishing a periodic flow to confirm it matches your expectations.
...
For Weekly flows, the first send is the next occurrence of a day selected under **Repeat on** that falls on or after your Start Date. It is not necessarily a full **Repeat every** interval away from the Start Date.

For example, if your Start Date is a Wednesday but you've selected Monday and Friday under Repeat on, the first send happens on the very next Monday or Friday after the Start Date. The full "Repeat every N weeks" cadence only takes effect starting from that first send. It doesn't govern how soon the first send happens.