  ...
  <Tab title="At fixed time">
    ...
    <Info>
      **How the first send is calculated**
      ...
      For Daily, Weekly, and Monthly recurrence, the first send is the earliest occurrence of your chosen pattern that falls on or after your Start Date and Send Time. If the pattern's matching day has already occurred earlier in that cycle — before your Start Date — that occurrence is skipped, and the first send moves to the next matching cycle. It will not send retroactively, and it will not send on the Start Date unless the Start Date itself matches the pattern.

      You can always confirm the computed first (or next) send date using the confirmation message shown once you finish configuring the schedule.
      ...
      This confirmation line reflects the computed first (or next) send date based on your current configuration. Always check it before publishing.
    </Info>
