...
<Tabs>
  <Tab title="OpenSharing (Zero-Copy Sharing)">
    ...
    - OpenSharing exports are part of the Streams add-on. Contact your dedicated MoEngage CSM (customer success manager) to enable them for your account.
  </Tab>
...
<Tabs>
  ...
  <Tab title="Direct Export">
    ...
    **Step 2: Use a Unity Catalog Volume for Faster Exports (Recommended)**

    This step is optional. MoEngage can use a Unity Catalog volume to export data more efficiently. To use this method, create a dedicated volume and grant MoEngage access to it.

    Use the following code to create the volume and grant access:

    ```sql wrap
    -- Create the volume. This is a one-time activity, and Databricks provisions the storage.
    CREATE VOLUME IF NOT EXISTS <CATALOG_NAME>.<SCHEMA_NAME>.<VOLUME_NAME>;

    -- Grant MoEngage read and write access on the volume.
    GRANT READ VOLUME, WRITE VOLUME ON VOLUME <CATALOG_NAME>.<SCHEMA_NAME>.<VOLUME_NAME>
      TO `<GRANTEE>`;
    ```
    ...
    - `<CATALOG_NAME>` - The name of your catalog.
    - `<SCHEMA_NAME>` - The name of your database or schema.
    - `<VOLUME_NAME>` - The name of the volume you are creating.
    - `<GRANTEE>` - The service principal's application ID (if using OAuth or a Service Principal PAT) or the user's email address (if using a User PAT).

    Enter the volume name in the **Unity Catalog Volume** field when you add the export.

    **Step 3: Add the Export on the App Marketplace**
...
5. Enter the following details:
   ...
   | **Unity Catalog Volume** | Optional | Enter the name of the volume you created for MoEngage. Leave it empty if you are not using a volume. To create one and grant MoEngage access, refer to the **Direct Export** tab under [Set Up Your Export](/user-guide/data/exports/events/data-warehouse/databricks-exports#set-up-your-export). |
   ...
   ![Setup exports to Databricks form showing the export name, Databricks connection, export into schema, Unity Catalog Volume, user properties, and event selection fields, with the Save button highlighted](/images/databricks-add-export-form.png)
