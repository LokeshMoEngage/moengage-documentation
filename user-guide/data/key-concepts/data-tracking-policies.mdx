...
2. For sending an attribute of datatype - datetime, please use the ISO 8601 date format (e.g. 2020-05-31T16:33:35Z) while sending data from our Data APIs and use our SDK date functions when sending data from the SDK directly. The SDK does not infer the datatype from the value, so a date sent from the SDK as a plain string is stored as a string even when it is in valid ISO 8601 format. Segmentation then offers the string operators for that attribute rather than the date operators. Please check with your Customer success team in case you are not able to send a certain ISO 8601 supported formats, the most commonly accepted formats are %Y-%m-%dT%H:%M:%S.%fZ; %Y-%m-%dT%H:%M:%SZ. Datetime values must be specified in GMT (UTC). Only GMT timestamps are accepted, any other offset passed are not consumed as dates.
