...
If the bounce is permanent, it's called a hard bounce. It typically happens when the recipient's domain doesn't exist or has been permanently disabled, or the recipient's email address does not exist. If the user's email ID is later changed, the Hard Bounce value is reset to false.
