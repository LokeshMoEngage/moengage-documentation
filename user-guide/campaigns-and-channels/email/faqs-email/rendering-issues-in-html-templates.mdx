...
<Warning>
Support for `prefers-color-scheme` depends entirely on the receiving email client, not on MoEngage. Gmail (on all platforms, including Android) and some Outlook versions do not honor this media query. Instead, these clients apply their own automatic dark-mode inversion to your template's colors and images, overriding any custom dark-mode CSS you've written.
</Warning>

Check [caniemail.com](https://www.caniemail.com/features/css-at-media-prefers-color-scheme/) for the current client support matrix before relying on `prefers-color-scheme` for critical branding elements like logos.
...
This avoids relying on CSS switching entirely, since it works with Gmail's auto-invert rather than fighting it.

<Note>
As a last-resort, advanced option, you can use a `mix-blend-mode: screen` CSS trick to force a white logo to display on inverted dark backgrounds. This requires careful cross-client testing and is not guaranteed to work in every environment. Check support for `mix-blend-mode` at [caniemail.com](https://www.caniemail.com/features/css-mix-blend-mode/) before relying on this technique.
</Note>
...
<Info>
General design tip: choose logo and text colors (for example, grey or brand colors with sufficient contrast in both modes) that remain legible whether or not dark-mode CSS is honored by the recipient's client.
</Info>
