...
MoEngage API requests require Basic Authentication. Data APIs also support OAuth 2.0; for more information, refer to [OAuth 2.0](/api/data/data-overview#oauth-2). To authenticate using Basic Authentication, include a Base64-encoded string of your credentials in the Authorization header of every request. Basic Authentication encodes a 'username:password' string in Base64 and prefixes the encoded string with 'Basic '. This string is included in the Authorization header as shown below:
...
For creating, scoping, regenerating, and archiving keys, see [API Key Dashboard](/user-guide/settings/account/api-and-api-keys/api-key-dashboard).
